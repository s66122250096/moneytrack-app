...
import { Plus, Trash2, X, ArrowLeft } from 'lucide-react-native';
...
export function CategoryScreen({ navigation }) {
  ...
  const handleDelete = (name) => {
    if (name === 'อื่นๆ') {
      Alert.alert('ไม่สามารถลบได้', 'หมวดหมู่ "อื่นๆ" ไม่สามารถลบได้');
      return;
    }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          ...
          onPress={() => { setNewCat(''); setAddModal(true); }}
        >
        ...
        {[{ key: 'expense', label: 'รายจ่าย' }, { key: 'income', label: 'รายรับ' }].map(t => (
          ...
          >
            <Text style={[styles.tabText, tab === t.key && styles.tabTextActive]}>
              {t.label} ({categories[t.key]?.length || 0})
            </Text>
          </TouchableOpacity>
        ))}
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptySubText}>กด + เพื่อเพิ่มหมวดหมู่</Text>
            </View>
          }
        />
      )}
        ...
        <View style={styles.overlay}>
          <View style={styles.modalBox}>
            ...
            <TextInput
              ...
              placeholder="เช่น ท่องเที่ยว, ออมเงิน..."
...
const styles = StyleSheet.create({
  ...
  addButton: {
    backgroundColor: '#10b981', borderRadius: 12, padding: 10,
    shadowColor: '#10b981', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.3, shadowRadius: 6, elevation: 3,
  },

  tabRow: {
    flexDirection: 'row', marginHorizontal: 20, backgroundColor: '#ffffff',
    borderRadius: 16, padding: 4, gap: 4, marginBottom: 16,
    shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4, elevation: 1,
  },
  ...
  catName: { flex: 1, fontSize: 15, fontWeight: '500', color: '#1f2937' },
  deleteBtn: { padding: 6, width: 32, alignItems: 'center' },
  ...
  modalInput: {
    ...
    paddingHorizontal: 16, paddingVertical: 12, fontSize: 15, color: '#1f2937', marginBottom: 4,
  },
