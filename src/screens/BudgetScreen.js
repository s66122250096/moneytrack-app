...
export function BudgetScreen() {
  ...
  const loadData = async () => {
    ...
    try {
      const now = new Date();
      const [budget, transactions] = await Promise.all([getBudget(user.uid), getTransactionsByMonth(user.uid, now.getFullYear(), now.getMonth() + 1)]);
      const { expense } = calcSummary(transactions);
      const byCategory = calcExpenseByCategory(transactions).map((item, i) => ({
        ...item,
        budget: Math.round(budget * 0.3), // สมมติแบ่งงบประมาณเป็น 30% สำหรับแต่ละหมวด (ปรับได้ตามต้องการ)
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={() => (editingBudget ? handleSaveBudget() : setEditingBudget(true))}>{editingBudget ? <Check size={22} color="#10b981" /> : <Pencil size={22} color="#10b981" />}</TouchableOpacity>
        </View>

        <View style={[styles.budgetCard, { backgroundColor: isOver ? "#dc2626" : isWarning ? "#ef4444" : "#10b981" }]}>
          ...
          {editingBudget ? (
            <View style={styles.editRow}>
              ...
              <TextInput style={styles.budgetInput} value={editValue} onChangeText={setEditValue} keyboardType="numeric" autoFocus selectTextOnFocus />
            </View>
          ) : (
          ...
          <View style={styles.remainingRow}>
            ...
            {(isOver || isWarning) && (
              <View style={styles.warningBadge}>
                ...
                <Text style={styles.warningText}>{isOver ? "เกินงบแล้ว!" : "ใกล้เกินงบ!"}</Text>
              </View>
            )}
          </View>
        </View>
                ...
                return (
                  <View key={item.name} style={styles.categoryCard}>
                    <View style={styles.categoryCardHeader}>
                      ...
                      <Text style={[styles.categoryStatus, { color: isOver ? "#dc2626" : isNear ? "#ea580c" : "#6b7280" }]}>฿{item.value.toLocaleString()}</Text>
                    </View>
